...
> **R:** Porque programa.i es el resultado del preprocesador de C, no el código original.
...
> **R:**Porque en programa.i los comentarios ya fueron eliminados por el preprocesador.
...
> **R:** Las líneas `# N "archivo"` indican el número de línea y el archivo de origen del código que está siendo incluido o procesado por el preprocesador. Sirven para que el compilador y el debugger puedan rastrear de qué archivo real proviene cada parte del código (especialmente en includes), en lugar de mostrar solo el archivo `.i`. El bloque que contiene la declaración de `printf` proviene del archivo: C:/msys64/ucrt64/include/stdio.h
...
> **R:**push    %rbp mov     %rsp, %rbp mov     %edi, -4(%rbp) mov     %esi, -8(%rbp)

Explicá en términos generales qué hacen esas instrucciones
(usá los comentarios del laboratorio como guía):

> **R:** Estas instrucciones hacen esto: push %rbp:guarda el estado anterior de la pila. mov %rsp, %rbp: crea el “marco” de la función sumar. mov %edi, -4(%rbp): guarda el primer parámetro (a). mov %esi, -8(%rbp):  guarda el segundo parámetro (b)
...
> **R:** Porque en programa.o la función todavía no está implementada, solo está declarada como referencia externa. El archivo matematica.o es el que la implementa (la define realmente).
...
> **R:**Quedan símbolos U porque el ejecutable usa funciones de la biblioteca estándar de C (libc) que no se copian dentro del binario.
...
> **R:** Una macro función como CUADRADO(x) es una sustitución textual que realiza el preprocesador antes de compilar. No es una función real: el código se “copia y pega” donde se usa. Una función real como sumar(a, b) sí existe en tiempo de compilación y ejecución: tiene parámetros, tipo de retorno y genera código ensamblador.
...
> **R:** En nm: T: símbolo definido en la sección .text (código ejecutable, funciones).  D: símbolo definido en la sección .data (variables globales inicializadas)
...
> **R:** ntdll.dll => /c/Windows/SYSTEM32/ntdll.dll (0x7ffa20330000)
        KERNEL32.DLL => /c/Windows/System32/KERNEL32.DLL (0x7ffa1e3e0000)
        KERNELBASE.dll => /c/Windows/System32/KERNELBASE.dll (0x7ffa1dc90000)
        ucrtbase.dll => /c/Windows/System32/ucrtbase.dll (0x7ffa1db90000)

¿Por qué `libc` no hubo que especificarla explícitamente al enlazar con `gcc`?

> **R:** Porque gcc la agrega automáticamente durante el enlazado.
